...
def normalize(v):
    norm = np.linalg.norm(v,axis=1) + 1e-4
...
def main():
    ...
    logging.basicConfig(filename =  CFG.savedir+"/%d-linear-linear-fine-tune-%s-%s.log"%(CFG.code_len, CFG.loss_type, CFG.sim_type),
                    level = logging.INFO,
...
#            break
    ...
    model_path = os.path.join(CFG.savedir,'pretrain_model.pth')
    model = torch.load(model_path)[0]#TODO read pretrain
    query_model = copy.deepcopy(model)
    base_model = torch.load(model_path)[0]
    ...
    if CFG.code_len > 0:
        ...
        bin_mlp = MLP(1600, 2000, 3000, 0, dropout=0)
        #bin_mlp = nn.Linear(1600, 2000)
        model_bin_head = BinarizeHead(2000+1600, CFG.code_len, bin_mlp, jump=True)
        #q_bin_mlp = nn.Linear(1600, 2000)#
        q_bin_mlp = MLP(1600, 2000, 3000, 0, dropout=0)
        qmodel_bin_head = BinarizeHead(2000+1600, CFG.code_len, q_bin_mlp, jump=True)
        #model_bin_head = nn.Linear(1600,1000)
        #qmodel_bin_head = nn.Linear(1600,1000)
        model_bin_head = BinarizeHead(1600, CFG.code_len, None)
        qmodel_bin_head = BinarizeHead(1600, CFG.code_len, None)
    ...
    model_decode_head = MLP(decode_n_input, 1600, 2000,2,0, last_act=False, jump=True)
    q_model_decode_head = MLP(decode_n_input, 1600, 2000,2,0, last_act=False, jump=True)

    model_decode_head = nn.Linear(decode_n_input, 1600)
    q_model_decode_head = nn.Linear(decode_n_input, 1600)

    #model, query_model, model_bin_head, qmodel_bin_head, model_decode_head, q_model_decode_head = torch.load(CFG.savedir+'qaea_fine_tune_%s_%s_hash_no-freeze-aux-10-linear-linear.pth'%(CFG.loss_type, CFG.sim_type))
    ...
    all_params = list(model_decode_head.parameters())+list(q_model_decode_head.parameters())
    if CFG.code_len > 0:
    ...
    if CFG.code_len > 0:
        ...
        #optimizer1 = torch.optim.Adam(list(model_bin_head.parameters())+list(model_decode_head.parameters())+list(model.parameters()), lr=CFG.lr, weight_decay=0)
        #optimizer2 = torch.optim.Adam(list(qmodel_bin_head.parameters())+list(q_model_decode_head.parameters())+list(query_model.parameters()), lr=CFG.lr, weight_decay=0)
        optimizer1 = torch.optim.Adam(list(model_bin_head.parameters())+list(model_decode_head.parameters()), lr=CFG.lr, weight_decay=0)
        optimizer2 = torch.optim.Adam(list(qmodel_bin_head.parameters())+list(q_model_decode_head.parameters()), lr=CFG.lr, weight_decay=0)
        ...
        optimizer1 = torch.optim.Adam(list(model_bin_head.parameters()), lr=CFG.lr, weight_decay=0)
        optimizer2 = torch.optim.Adam(list(qmodel_bin_head.parameters()), lr=CFG.lr, weight_decay=0)
        optimizer3 = torch.optim.Adam(list(model_decode_head.parameters())+list(q_model_decode_head.parameters()))
    ...
    query_data = [(a,b) for a,b in zip(query_data[0], query_data[1])]
    print('sample')
    query_data = random.sample(query_data, 500000)
    print('transform')
    aux_data_list = [(sequence_to_graph(0, seq, tar, tokenizer, CFG.query_max_len),
                        sequence_to_graph(0, seq+tar, tar, tokenizer, CFG.query_max_len)) for seq,tar in query_data]
    ...
    def tfm(data):
        return sequence_to_graph(0, data['ori_seq'][0], data['ori_seq'][0][:-1], tokenizer, CFG.query_max_len)
...
#        sim_head.train()
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            if i == len(train_loader) - 1 or i == 1:
                ...
                print(torch.mean(torch.abs(aux_sub_out)), torch.mean(torch.abs(aux_out)))
                #a = torch.sum((aux_sub_qaea_emb-rec_aux_sub_qaea)**2,dim=1)/aux_sub_norm
                #print(torch.max(a))
                #print(torch.median(a))
                #print(aux_sub_norm[a>1])
                #print(torch.sum((aux_sub_qaea_emb-rec_aux_sub_qaea)**2,dim=1)[a>1])
                #print(aux_sub_qaea_emb[a>1,:])
                #print(rec_aux_sub_qaea[a>1,:])
            ...
            try:
                ...
                a = aux_sub_out
                b = aux_out
                aux_base_pred = F.normalize(a) @ F.normalize(b).T
                aux_base_pred = aux_base_pred.detach()
                ...
                aux_loss = torch.mean((aux_pred-aux_base_pred)**2)
                ...
                #loss = CFG.aux_w*aux_loss + CFG.bin_w*reg_loss
                #loss = 0
                rec_aux_sub_qaea = q_model_decode_head(aux_sub_out)
                rec_aux_qaea = model_decode_head(aux_out)
                #rec_aux_sub_qaea = aux_sub_out
                #rec_aux_qaea = aux_out
                aux_sub_qaea_emb = 1 * base_model(aux_sub_data).detach()
                aux_qaea_emb = 1 * base_model(aux_data).detach()
                #if i % 2 == 0:
                    #print(torch.mean(torch.abs(aux_sub_qaea_emb-a)))
                #    assert(torch.mean(torch.abs(aux_sub_qaea_emb-a))<1e-6)
                #else:
                    #print(torch.mean(torch.abs(aux_qaea_emb-b)))
                #    assert(torch.mean(torch.abs(aux_qaea_emb-b))<1e-6)
                ...
                #loss = 0
                aux_sub_norm = torch.sum(aux_sub_qaea_emb**2, dim=1).detach()
                aux_norm = torch.sum(aux_qaea_emb**2, dim=1).detach()
                if i % 2 == 0:
                    #aux_norm = 1
                    rec_loss = torch.mean(torch.sum((aux_qaea_emb-rec_aux_qaea)**2, dim=1)/aux_norm ) 
                    rec_loss += 0 - torch.mean(F.cosine_similarity(aux_qaea_emb, rec_aux_qaea))
                else:
                    #aux_sub_norm = 1
                    rec_loss = torch.mean(torch.sum((aux_sub_qaea_emb-rec_aux_sub_qaea)**2,dim=1)/aux_sub_norm)
                    rec_loss += 0 - torch.mean(F.cosine_similarity(aux_sub_qaea_emb, rec_aux_sub_qaea))
            ...
            training_loss.append([loss.item(), rec_loss.item(), aux_loss.item()])
            train_reg_loss.append(aux_loss.item())
        ...
        if True:#i % CFG.ckpt_iter == 0:
            ...
            with torch.no_grad():
                ...
                #reg_loss = torch.mean(torch.abs(mat-reg_tgt))
                    aux_sub_out = query_model(aux_sub_data)
                    aux_out = model(aux_data)
                    aux_base_pred = F.normalize(aux_sub_out) @ F.normalize(aux_out).T
                    aux_base_pred = aux_base_pred.detach()
                    if CFG.code_len > 0:
                        ori_out = qmodel_bin_head(ori_out)
                        pos_out = model_bin_head(pos_out)
                    ...
                    #rec_aux_sub_qaea = aux_sub_out
                    #rec_aux_qaea = aux_out
                    aux_sub_qaea_emb = 1 * base_model(aux_sub_data)
                    aux_qaea_emb = 1 * base_model(aux_data)
                
                    aux_sub_norm = torch.sum(aux_sub_qaea_emb**2, dim=1).detach()
                    aux_norm = torch.sum(aux_qaea_emb**2, dim=1).detach()
                    
                    rec_loss = torch.mean(torch.sum((aux_sub_qaea_emb-rec_aux_sub_qaea)**2,dim=1)/aux_sub_norm)+torch.mean(torch.sum((aux_qaea_emb-rec_aux_qaea)**2, dim=1)/aux_norm)
            ...
            if ave_valid_loss < best_valid_loss:
                print('save')
                if CFG.code_len > 0:
                    torch.save((model, query_model, model_bin_head, qmodel_bin_head, model_decode_head, q_model_decode_head), CFG.savedir+"qaea_fine_tune_%s_%s_hash_%d-linear-linear.pth"%(CFG.loss_type, CFG.sim_type, CFG.code_len))
                else:
...
def test():
    ...
    if CFG.code_len > 0:
        model, query_model, model_bin_head, qmodel_bin_head, model_decode_head, qmodel_decode_head = torch.load(CFG.savedir+'qaea_fine_tune_%s_%s_hash_%d-linear-linear.pth'%(CFG.loss_type, CFG.sim_type, CFG.code_len))
    else:
   ...
   # query_model = QAEA_Linear(768)
    ...
    train_list = [sequence_to_graph(0, seq, seq[:1], tokenizer, CFG.query_max_len) for seq in train_data]
    ...
    query_list = [sequence_to_graph(0, seq, seq[:1], tokenizer, CFG.query_max_len) for seq, _ in zip(test_data[0], test_data[1])]
    ...
    with torch.no_grad():
        ...
        for d_data in tqdm(db_loader):
            ...
            d_qaea_emb = model(d_data.to(device))
            if CFG.code_len > 0:
        ...
        decoded_qaea_emb = np.concatenate(decoded_qaea_emb_list, axis=0) / 50
        ...
        for q_data in tqdm(query_loader):
            ...
            q_qaea_emb = query_model(q_data.to(device))
            if CFG.code_len > 0:
        ...
        decoded_q_qaea_emb = np.concatenate(decoded_q_qaea_emb_list, axis=0) / 50
        ...
        if CFG.code_len > 0:
            ...
            D,I = index.search(normalize(q_emb), 100)
        gt = np.zeros_like(I,dtype=np.float32)
        for i,t in enumerate(test_data):
            for j,d in enumerate(I[i,:]):
                r = train_data[d]
